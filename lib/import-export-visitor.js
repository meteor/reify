    ...
    for (let i = 0; i < infoCount; ++i) {
      ...
      if (
        ...
        if (hasImports) {
          ...
          // Even though we don't need to wait for deps since there are none,
          // we still need to call the function to indicate that linking is done
          codeToInsert += `__reifyWaitForDeps__();`;
        }
    ...
    if (
      ...
      if (this.magicString !== null) {
        if (this.wrapForTopLevelAwait) {
          this.magicString.prepend('!module.wrapAsync(async function (' + this.moduleAlias + ', __reifyWaitForDeps__, __reifyAsyncResult__) {"use strict"; try {');
          this.magicString.append(`//*/\n__reifyAsyncResult__();} catch (_reifyError) { __reifyAsyncResult__(_reifyError); }}, { self: this, async: ${this.hasTopLevelAwait} });`);
        } else {
