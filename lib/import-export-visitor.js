    ...
    if (
      ...
      if (this.magicString !== null) {
        if (this.prepareForTopLevelAwait) {
          ...
          let functionWrapper = this.hasTopLevelAwait ? 'async function' : 'function';
          this.magicString.prepend('!module.wrapAsync(prepare, this, ' + functionWrapper + '() {');
          this.magicString.append('//*/\n}); function prepare() { return [');
          const requires = this.imports.map(source => {
            return `require(${source})`;
          });
          this.magicString.append(requires.join(',\n'));
          this.magicString.append(']};');
        } else {
