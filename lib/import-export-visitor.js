...
class ImportExportVisitor extends Visitor {
  finalizeHoisting() {
    ...
    for (let i = 0; i < infoCount; ++i) {
      ...
      if (this.prepareForTopLevelAwait && bodyInfo.parent.type === 'Program') {
  ...
  maybeWrapModuleWithFunction() {
    if (
      ...
      if (this.magicString !== null) {
        if (this.prepareForTopLevelAwait) {
          // TODO: add module alias
          this.magicString.prepend('!module.wrapAsync(async function (module, __reifyWaitForDeps__, __reify_async_result__) {');
