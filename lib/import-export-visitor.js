...
class ImportExportVisitor extends Visitor {
  finalizeHoisting() {
    ...
    for (let i = 0; i < infoCount; ++i) {
      ...
      let addUseStrict = bodyInfo.needToAddUseStrictDirective &&
        (this.madeChanges || isModule)

      if (!addUseStrict && this.code &&
      ...
      if (
        this.prepareForTopLevelAwait &&
        bodyInfo.parent.type === 'Program'
        && (hasImports || this.hasTopLevelAwait)
        && !this.wrapForTopLevelAwait
      ) {
        this.wrapForTopLevelAwait = true;
        // TODO: we probably can skip adding this code if there are no imports
        // but for some reason it breaks many of the tests
        codeToInsert += `if (__reifyWaitForDeps__()) (await __reifyWaitForDeps__())();`
      } else if (addUseStrict) {
        // All modules that are wrapped for top level await (use imports or 
        // have top level await) have "use strict" added when wrapped
        // so we skip them here.

        codeToInsert = '"use strict";' + codeToInsert;
      }
  ...
  maybeWrapModuleWithFunction() {
    if (
      ...
      if (this.magicString !== null) {
        if (this.wrapForTopLevelAwait) {
          this.magicString.prepend('!module.wrapAsync(async function (' + this.moduleAlias + ', __reifyWaitForDeps__, __reify_async_result__) {"use strict"; try {');
          // TODO: we can change this to remove the return, such as by moving
          // the second call to inside the try block
          this.magicString.append(`//*/\n} catch (_reifyError) { return __reify_async_result__(_reifyError); } __reify_async_result__(); }, { self: this, async: ${this.hasTopLevelAwait} });`);
        } else {
                ...
                arguments: [
                  {
                    ...
                    params: [
                      id(this.moduleAlias),
                      id('__reifyWaitForDeps__'),
                      id('__reify_async_result__')
                    ],
                    body: {
                      type: "BlockStatement",
                      body: [
                              ...
                              body: [
                                {
                                  ...
                                  argument: {
                                    type: 'CallExpression',
                                    callee: id('__reify_async_result__'),
                                    arguments: [
                                      id('_reifyError')
                                    ]
                                  }
                  ...
                  },
                  {
                    ...
                    properties: [
                      ...
                      },
                      {
                        ...
                        value: { type: "BooleanLiteral", value: this.hasTopLevelAwait }
                      }
                    ]
                  }
                ]
  ...
  reset(rootPath, codeOrNull, options) {
    ...
    this.wrapForTopLevelAwait = getOption(options, "forceWrapForTopLevelAwait");
    ...
    this.prepareForTopLevelAwait = getOption(options, "topLevelAwait");
    this.hasTopLevelAwait = getOption(options, "forceHasTopLevelAwait");
  ...
  visitImportDeclaration(path) {
    ...
    let source = getSourceString(this, decl);
  ...
  visitImportExpression(path) {
    if (this.dynamicImport) {
      ...
      let end = importCallee.start + 6;
      overwrite(this, importCallee.start, end, replacement);
...
// Babel uses Import instead of ImportExpression for the node type
ImportExportVisitor.prototype.visitImport =
  ImportExportVisitor.prototype.visitImportExpression;
