...
Ep.addAsyncDep = function (childEntry) {
  ...
  childEntry.asyncParents.push(this);
}
  ...
  switch(status) {
    ...
    case 'evaluated':
      ...
      toEvaluate.forEach(parent => {
        // Propagate the error up the module graph
        if (this.evaluationError && !parent.evaluationError) {
          parent.evaluationError = this.evaluationError;
        }
...
Ep.gatherReadyAsyncParents = function (readyList) {
  this.asyncParents.forEach(function (parent) {
    // TODO: remove assertions
    // assert.equal(parent.status, 'evaluating-async');
    assert.equal(parent.evaluationError, null);
    if (parent.pendingAsyncDeps === 0) {
      console.log(parent);
    }
    assert.equal(parent.pendingAsyncDeps > 0, true);
    assert.equal(parent.asyncEvaluation, true);
