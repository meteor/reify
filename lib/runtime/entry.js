...
Ep.addAsyncDep = function (childEntry) {
  ...
  let self = this;
  // console.log('add child', self.id, childEntry.id)
  childEntry._onEvaluated.push(function (notifyOnly) {
    // console.log('child finished', self.id, childEntry.id)
    if (notifyOnly) {
      self.asyncDeps -= 1;
      return;
    }
...
Ep.changeStatus = function (status) {
  switch(status) {
    ...
    case 'evaluated':
      ...
      // These functions are called twice. Once to inform them
      // this module was evaluated, and the second time
      // so they can perform whatever action they want to
      // after this module finished.
