...
function wrapAsync(body, options) {
  ...
  let hasTopLevelAwait = options.async;
  ...
  body.call(options.self, module, function waitForDeps() {
    ...
    if (waitForDepsResult === undefined) {
      ...
      entry.hasTopLevelAwait = hasTopLevelAwait;
      entry.asyncEvaluation = hasTopLevelAwait || entry.pendingAsyncDeps > 0;
      ...
      if (entry.pendingAsyncDeps === 0) {
        ...
        if (!entry.hasTopLevelAwait && entry.asyncEvaluation) {
        ...
        entry._onEvaluating.push(function () {
          ...
          if (entry.asyncEvaluation && !entry.hasTopLevelAwait) {
            entry.changeStatus('evaluated');
          }
        });
      }
