...
function wrapAsync(prepare, moduleThis, moduleContent) {
  ...
  function exportPromise() {
    ...
    promiseExports[module.id] = promise;

    promise.__onEvaluated = onEvaluated;
  }
  ...
  function isAsyncModule(exports) {
    return isPromise(exports) && exports.__onEvaluated;
  }
  ...
  function prepareModule() {
    ...
    for (let i = 0; i < prepareResult.length; i++) {
      if (isAsyncModule(prepareResult[i])) {
        hasPromises = true;
        pendingDeps += 1;

        prepareResult[i].__onEvaluated.push(function () {
          pendingDeps -= 1;
          if (pendingDeps === 0) {
            evaluateModule();
          }
  ...
  function finish() {
    ...
    onEvaluated.forEach(function (callback) {
      callback();
    });
  }
