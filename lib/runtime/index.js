...
exports.enable = function (mod) {
  if (mod.link !== moduleLink) {
    ...
    mod.require = function (id) {
      ...
      var exports = origRequire.call(this, path);
...
function moduleLink(id, setters, key) {
  ...
  if (childEntry.asyncEvaluation && parentEntry.status !== 'linking') {
    // TODO: this prevents dynamic imports from working in Meteor
    // throw new Error('Nested imports can not import an async module');
  }

  if (childEntry.asyncEvaluation && childEntry.status !== 'evaluated') {
