...
exports.compile = function (code, options) {
  ...
  let likelyIdentifiers = ['import', 'export'];

  if (getOption(options, 'topLevelAwait')) {
    likelyIdentifiers.push('await');
  }
  ...
  options.possibleIndexes =
    utils.findLikelyIndexes(code, likelyIdentifiers);
  ...
  options.topLevelAwait = getOption(options, 'topLevelAwait');
